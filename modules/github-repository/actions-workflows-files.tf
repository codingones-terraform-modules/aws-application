...
resource "github_repository_file" "s3_sync_and_invalidate_cache" {
  ...
  file                = ".github/workflows/build-sync-and-invalidate-cache.aws.yml"
...
module "workflow_template" {
  ...
  template_url = "https://raw.githubusercontent.com/codingones/github-files-templates/main/github-actions/build-sync-and-invalidate-cache.aws.yml"
  template_variables = {
    SERVICE      = var.service
